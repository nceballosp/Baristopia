                ...
                <div class="card-body">
                    <div class="row">
                        <div class="col-md-6">
                            <h5>{{ $map->name }}</h5>
                            <p>{{ $map->description }}</p>
                            <p><strong>{{ __('messages.location') }}</strong> X={{ $map->left }}%, Y={{ $map->top }}%</p>
                        </div>
                        <div class="col-md-6">
                            <div>
                    ...
                    <div class="mt-3">
                        <a href="{{ route('map.index') }}" class="btn btn-secondary">{{ __('messages.return') }}</a>
                        <form action="{{ route('map.delete') }}" method="POST" class="d-inline">
                            @csrf
                            @method('DELETE')
                            <input type="hidden" name="id" value="{{ $map->id }}">
                            <button type="submit" class="btn btn-danger" onclick="return confirm({{ __('messages.rmCafe') }})">{{ __('messages.remove') }}</button>
                        </form>
                    </div>
